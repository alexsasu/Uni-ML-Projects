...
# Collecting the train, validation, and test data in order to feed it to the model

X_train = padded_train_sequences
X_val = padded_val_sequences
...
# Text classifier
def build_text_classifier(vocab_size, embedding_dim, max_seq_length):
    ...
    text_features = Dense(1, activation='sigmoid', name="Classifier")(text_features)
    return Model(inputs=text_input, outputs=text_features, name="Text_Classifier")

# --------------------------------------------------

# We build and compile our model

model = build_text_classifier(vocab_size, embedding_dim, max_seq_length)
