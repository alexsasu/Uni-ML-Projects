...
# Modelul RandomForest + GridSearchCV
##########################################################################################
# Configuram modelul
RandomForest = RandomForestClassifier(random_state=8)
forestParameters = {'n_estimators': [16, 32, 64, 128, 256], 'criterion': ["gini", "entropy"]}
GridSearch = GridSearchCV(estimator=RandomForest, param_grid=forestParameters)

# Antrenam modelul
GridSearch.fit(X=train_images, y=train_labels)

# Obtinem label-urile prezise de catre model
predicted_labels.extend(GridSearch.predict(X=validation_images))
...
RandForestAndGridSearch_accuracy = accuracy_score(y_true=validation_labels, y_pred=predicted_labels)
print(f"\nRandomForest + GridSearchCV accuracy: {RandForestAndGridSearch_accuracy}\n")

# Obtinem matricea de confuzie pentru predictiile generate de catre clasificator
RandForestAndGridSearch_cm = confusion_matrix(y_true=validation_labels, y_pred=predicted_labels)
print(f"RandomForest + GridSearchCV confusion matrix:\n{RandForestAndGridSearch_cm}")
##########################################################################################