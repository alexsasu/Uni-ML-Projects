...
# Modelul KNN
##########################################################################################
# Configuram si antrenam modelul
KNN = KNeighborsRegressor(n_neighbors=2).fit(X=train_images, y=train_labels)

# Obtinem label-urile prezise de catre model
predicted_labels.extend(KNN.predict(X=validation_images))
...
KNN_accuracy = accuracy_score(y_true=validation_labels, y_pred=predicted_labels)
print(f"\nKNN accuracy: {KNN_accuracy}\n")

# Obtinem matricea de confuzie pentru predictiile generate de catre clasificator
KNN_cm = confusion_matrix(y_true=validation_labels, y_pred=predicted_labels)
